...
- OpenCode: selects a mature stable `vMAJOR.MINOR.PATCH` GitHub Release, downloads the upstream installer from a pinned revision, verifies its SHA-256, and installs that exact version in the dedicated persistent `opencode-install` volume
...
- pnpm installs run with `ignore-scripts=true`. OpenCode does not use pnpm: after source-hash verification, AGS runs the upstream installer with `HOME=/opt/opencode-home`, `--version <selected-tag>`, and `--no-modify-path`; it validates and runs `/opt/opencode-home/.opencode/bin/opencode` directly. The former `opencode-ai` package is removed only as migration cleanup.
