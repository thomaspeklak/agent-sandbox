...
- npm/pnpm lifecycle scripts are disabled in the sandbox (`ignore-scripts=true`). For OpenCode, AGS selects a mature stable `vMAJOR.MINOR.PATCH` GitHub Release, verifies the source hash of the upstream installer pinned to an exact revision, installs that version in a dedicated persistent volume outside pnpm, and executes its binary for validation.
